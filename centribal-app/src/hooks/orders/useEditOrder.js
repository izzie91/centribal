...
const useEditOrder = ({ langLabels }) => {
  ...
  const getOrderById = useCallback(async () => {
    await orderService.fetchOrderById(orderId, langLabels.ErrorMessge);
  ...
  const getProducts = useCallback(async () => {
    await productService.fetchProducts(langLabels.ErrorMessge);
    setProducts(productService.getProducts());
  }, []);

  const editOrderById = useCallback(
    async (data) => {
      await orderService.updateOrder(orderId, data, langLabels.updatedMessage, langLabels.ErrorMessge);
